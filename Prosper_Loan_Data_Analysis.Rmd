...
listing_category <- c("Not Available", "Debt Consolidation",
...
loan$ListingCategoryName <- 
  listing_category[(loan$ListingCategory..numeric.)+1]
#Convert to factor
...
ggplot(aes(LoanOriginalAmount, fill = LoanStanding), data = loan) +
  ...
  theme(legend.title = element_text(size = 9),
        ...
        legend.position = c(1,1),
        axis.text.x = element_text(size = 9, color = 'grey0'), 
        plot.title = element_text(family = 'Times', face = 'bold.italic',
...
condition <- c("Employed", "Part-time", "Full-time",
               "Not employed", "Self-employed")

#Plot 1
p1 <- ggplot(subset(loan, EmploymentStatus %in% condition &
...
#Plot 2
p2 <- ggplot(subset(loan, EmploymentStatus %in% condition &
                      !is.na(EmploymentStatusDuration)), 
...
#Plot 3
p3 <- ggplot(subset(loan, EmploymentStatus %in% condition &
                      !is.na(EmploymentStatusDuration)), 
             aes(EmploymentStatusDuration)) + 
  geom_histogram() +
  scale_x_log10() +
...
#Summary
summary(subset(loan, EmploymentStatus %in% condition &
                 !is.na(EmploymentStatusDuration))$EmploymentStatusDuration)
```
...
loan$IncomeRange <- factor(loan$IncomeRange, levels = c('$0', '$1-24,999',
                                                        '$25,000-49,999',
                                                        '$50,000-74,999',
                                                        '$75,000-99,999',
                                                        '$100,000+'))
#Plot
ggplot(aes(IncomeRange, fill = LoanStanding),
       data = subset(loan,
                     IncomeVerifiable == 'True' & !is.na(IncomeRange))) +
  geom_bar(width = .7) +
  ylab('Number of Borrowers') +
...
CreditScoreDifference <- table(loan$CreditScoreRangeUpper -
...
#Plot
ggplot(aes(DebtToIncomeRatio, fill = LoanStanding),
...
#Plot
ggplot(aes(BorrowerAPR, fill = LoanStanding),
       data = subset(loan, !is.na(BorrowerAPR))) +
  geom_area(stat = 'bin', binwidth = 0.005) +
  coord_cartesian(xlim = c(0,
                           quantile(loan$DebtToIncomeRatio, 0.90, na.rm = T))) +
  ylab('Number of Borrowers') +
...
#Plot
ggplot(aes(ProsperScore, fill = LoanStanding),
       data = subset(loan, !is.na(ProsperScore))) +
  geom_bar(position = 'dodge') +
  geom_text(stat = 'count', aes(label = ..count..),size = 2.5,
...
loan$ProsperRating <- factor(loan$ProsperRating,
...
#Plot
ggplot(aes(ProsperRating, fill = LoanStanding),
       data = subset(loan, !is.na(ProsperRating))) +
  geom_bar(position = 'dodge') +
...
loan$ProsperRiskRating <- ifelse(loan$ProsperScore == "1" |
                                 ...
                                 ifelse(loan$ProsperScore == "5" |
                                          ...
                                          loan$ProsperRating == "B", 'Medium',
                                        'Low'))
#Convert to Factor 
loan$ProsperRiskRating <- factor(loan$ProsperRiskRating,
...
#Plot
ggplot(aes(ProsperRiskRating, fill = LoanStanding),
       data = subset(loan, !is.na(ProsperRiskRating))) +
  geom_bar(position = 'dodge') +
  geom_text(stat = 'count', aes(label = ..count..),
            position = position_dodge(width = 0.9), size = 3.5, vjust = -0.4) +
  ylab('Number of Borrowers') +
...
loan$BorrowerStateName <-
  tolower(state.name[match(loan$BorrowerState, state.abb)])
...
ggplot() + 
  geom_map(data = us, map = us, aes(x = long, y = lat,
                                    map_id = region, label = region),
            fill="#ffffff", color="#ffffff", size=0.15) +
  geom_map(data = grp_by_state, map=us, aes(fill = count,
                                            map_id = BorrowerStateName),
            color = "#ffffff", size = 0.15) +
  geom_text(data = states, 
            aes(x = x, y = y, label = state.abb, group = NULL),
            size = 2) +
  scale_fill_continuous('Number of Borrowers',
                        low = 'azure2', high = 'dodgerblue4') +
...
grp_by_state_LoanStanding_wide$proportion_BadStanding <- 
  round((grp_by_state_LoanStanding_wide$Bad/
           (grp_by_state_LoanStanding_wide$Bad +
              grp_by_state_LoanStanding_wide$Good))*100,2) 
...
ggplot() + 
  geom_map(data = us, map = us, aes(x = long, y = lat,
                                    map_id = region, label = region),
            fill="#ffffff", color="#ffffff", size=0.15) +
  geom_map(data = grp_by_state_LoanStanding_wide, map=us,
           aes(fill = proportion_BadStanding, map_id = BorrowerStateName),
            color = "#ffffff", size = 0.15) +
  geom_text(data = states, 
            aes(x = x, y = y, label = state.abb, group = NULL),
            size = 2) +
...
as.table(by(grp_by_state_LoanStanding_wide$proportion_BadStanding,
            grp_by_state_LoanStanding_wide$BorrowerStateName, sum))
```
...
grp_by_state_ProsperRisk_wide <- dcast(grp_by_state_ProsperRisk,
                                    BorrowerStateName +
                                      BorrowerState ~ ProsperRiskRating,
                                    value.var = 'count')
...
grp_by_state_ProsperRisk_wide$proportion_HighRisk <- 
  round((grp_by_state_ProsperRisk_wide$High/
           (grp_by_state_ProsperRisk_wide$High +
              grp_by_state_ProsperRisk_wide$Medium +
...
grp_by_state_ProsperRisk_wide$proportion_LowRisk <- 
  round((grp_by_state_ProsperRisk_wide$Low/
           (grp_by_state_ProsperRisk_wide$Low +
              grp_by_state_ProsperRisk_wide$Medium +
...
ggplot() + 
  geom_map(data = us, map = us, aes(x = long, y = lat,
                                    map_id = region, label = region),
            fill="#ffffff", color="#ffffff", size=0.15) +
  geom_map(data = grp_by_state_ProsperRisk_wide, map=us,
           aes(fill = proportion_HighRisk, map_id = BorrowerStateName),
            color = "#ffffff", size = 0.15) +
  geom_text(data = states, 
            aes(x = x, y = y, label = state.abb, group = NULL),
            size = 2) +
...
as.table(by(grp_by_state_ProsperRisk_wide$proportion_HighRisk,
            grp_by_state_ProsperRisk_wide$BorrowerStateName, sum))
...
ggplot() + 
  geom_map(data = us, map = us, aes(x = long, y = lat,
            ...
            fill="#ffffff", color="#ffffff", size=0.15) +
  geom_map(data = grp_by_state_ProsperRisk_wide, map=us,
           aes(fill = proportion_LowRisk, map_id = BorrowerStateName),
            color = "#ffffff", size = 0.15) +
  geom_text(data = states, 
            aes(x = x, y = y, label = state.abb, group = NULL),
            size = 2) +
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5, family = 'Times',
                                  face = 'bold.italic', color = 'black'))
#Tabulate the summary findings
as.table(by(grp_by_state_ProsperRisk_wide$proportion_LowRisk,
            grp_by_state_ProsperRisk_wide$BorrowerStateName, sum))
```
...
drop = c("ListingCategory..numeric.", "CreditScoreRangeLower",
         "CreditScoreRangeUpper")
loan <- loan[ , !(names(loan) %in% drop)]

#Plot Matrix
ggpairs(subset(loan, select = c(LoanStanding, LoanOriginalAmount,
                                EmploymentStatusDuration, BorrowerAPR,
                                DebtToIncomeRatio, CreditScore)))
```
...
#Plot matrix
ggpairs(subset(loan_2009, select = c(ProsperRiskRating, LoanOriginalAmount,
                                     EmploymentStatusDuration, BorrowerAPR,
                                     DebtToIncomeRatio, CreditScore)))
```
...
ggpairs(subset(loan_2009, select = -c(Term, LoanStatus, IncomeVerifiable,
                                 ...
                                 ListingCategoryName, ProsperScore,
                                 ProsperRating)))
```
...
b1 <- ggplot(aes(x = CreditScore, y = BorrowerAPR_mean),
             data = loan_APR_mean) +
  geom_line() +
...
b2 <- ggplot(aes(x = CreditScore, y = BorrowerAPR_median),
             data = loan_APR_mean) +
  geom_line() +
...
cor(loan$CreditScore, loan$BorrowerAPR,
...
c1 <- ggplot(aes(x = CreditScore, y = LoanOriginalAmount),
  ...
  coord_cartesian(xlim = c(quantile(loan$CreditScore,0.01, na.rm = T), 800),
                  ylim = c(quantile(loan$LoanOriginalAmount,0.01, na.rm = T),
                           quantile(loan$LoanOriginalAmount, 0.99,
                                    na.rm = T))) +
  theme_minimal() +
...
c2 <- ggplot(aes(x = CreditScore, y = LoanOriginalAmount_mean),
             data = loan_Amount_Mean) +
  geom_line() +
...
c3 <- ggplot(aes(x = CreditScore, y = LoanOriginalAmount_median),
             data = loan_Amount_Mean) +
  geom_line() +
...
cor(loan$CreditScore, loan$LoanOriginalAmount,
    use = 'complete.obs', method = 'pearson')
```
...
d1 <- ggplot(aes(x = CreditScore, y = DebtToIncomeRatio),
  ...
  geom_jitter(alpha = 1/20) +
  coord_cartesian(xlim = c(quantile(loan$CreditScore,0.01, na.rm = T), 800),
                  ylim = c(0,quantile(loan$DebtToIncomeRatio, 0.99,
                                      na.rm = T))) +
  theme_minimal() +
...
#Plot the mean and median
d2 <- ggplot(aes(x = CreditScore, y = DebtToIncomeRatio_mean),
             data = loan_ratio_mean) +
  geom_line() +
...
d3 <- ggplot(aes(x = CreditScore, y = DebtToIncomeRatio_median),
             data = loan_ratio_mean) +
  geom_line() +
...
#Calculate Correlation Coefficient
cor(loan$CreditScore, loan$DebtToIncomeRatio,
    use = 'complete.obs', method = 'pearson')
```
...
e1 <- ggplot(aes(x = IncomeRange, y = CreditScore),
       data = subset(loan, IncomeVerifiable == 'True' &
                       !is.na(IncomeRange) & !is.na(CreditScore))) + 
  geom_boxplot() + 
...
e2 <- ggplot(aes(x = IncomeRange, y = BorrowerAPR),
       data = subset(loan, IncomeVerifiable == 'True' &
                       !is.na(IncomeRange) & !is.na(BorrowerAPR))) + 
  geom_boxplot() + 
  stat_summary(fun.y=mean, geom="point", color = "red", shape = 1, size=2) +
...
ggplot(aes(x = ListingCategoryName, y = LoanOriginalAmount,
           color = ListingCategoryName), data = loan) +
  geom_boxplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 0.9, size = 8,
                                   colour = 'grey0'), legend.position = 'none',
        plot.title = element_text(hjust = 0.5, face = 'bold.italic',
                                  color = 'black', family = 'Times'))

#BorrowerAPR vs ListingCategory
ggplot(aes(x = ListingCategoryName, y = BorrowerAPR,
           color = ListingCategoryName), data = loan) +
  geom_boxplot() +
...
ggplot(aes(x = ListingCategoryName, y = CreditScore,
           color = ListingCategoryName),
       data = subset(loan, !is.na(loan$CreditScore))) +
  geom_boxplot() +
...
ggplot(aes(x= IsBorrowerHomeowner, y = LoanOriginalAmount), data = loan) +
  geom_boxplot() +
  coord_cartesian(ylim = c(0, quantile(loan$LoanOriginalAmount, 0.99,
                                       na.rm = T))) +
  scale_x_discrete(labels = c("No", "Yes")) +
...
ggplot(aes(x= IsBorrowerHomeowner, y = EmploymentStatusDuration),
       data = subset(loan, EmploymentStatus %in% condition &
                       !is.na(EmploymentStatusDuration))) +
  geom_boxplot() +
  coord_cartesian(ylim = c(quantile(loan$EmploymentStatusDuration, 0.01,
                                    na.rm = T),
                           quantile(loan$EmploymentStatusDuration, 0.99,
                                    na.rm = T))) +
  scale_x_discrete(labels = c("No", "Yes")) +
...
ggplot(aes(x= IsBorrowerHomeowner, y = IncomeRange),
  ...
  scale_x_discrete(labels = c("No", "Yes")) +
  scale_fill_gradient(low = 'lightgoldenrod', high = 'red4') +
...
Employment_condition <- c("Employed", "Full-time", "Part-time",
                          "Self-employed", "Retired", "Not employed")
#Plot
ggplot(aes(x = LoanStanding, y = EmploymentStatusDuration), 
       data = subset(loan,
                     EmploymentStatus %in% Employment_condition &
                       !is.na(EmploymentStatusDuration))) +
  geom_boxplot() +
  stat_summary(fun.y=mean, geom="point", color = "red", shape = 1, size=2) + 
  coord_cartesian(ylim = c(0, quantile(loan$EmploymentStatusDuration, 0.95,
                                       na.rm = T))) +
  theme_minimal() +
...
ggplot(aes(x = LoanStanding, y = DebtToIncomeRatio), data = loan) +
  geom_boxplot() +
  coord_cartesian(ylim = c(0, quantile(loan$DebtToIncomeRatio, 0.99,
                                       na.rm = T))) +
  stat_summary(fun.y=mean, geom="point", color = "red", shape = 1, size=2) +
  theme_minimal() +
...
ggplot(aes(x = ProsperRiskRating, y = EmploymentStatusDuration),
       data = loan_2009) +
  geom_boxplot() +
  stat_summary(fun.y=mean, geom="point", color = "red", shape = 1, size=2) +
  coord_cartesian(ylim = c(0, quantile(loan_2009$EmploymentStatusDuration,
                                       0.99, na.rm = T))) +
...
ggplot(aes(x = ProsperRiskRating, y = DebtToIncomeRatio), data = loan_2009) +
  geom_boxplot() +
  stat_summary(fun.y=mean, geom="point", color = "red", shape = 1, size=2) +
  coord_cartesian(ylim = c(0, quantile(loan_2009$DebtToIncomeRatio,0.95,
                                       na.rm = T))) +
...
loan_verifiable_income <- subset(loan, IncomeVerifiable == 'True' & 
                                   !is.na(IncomeRange) &
                                   IncomeRange != 'Not displayed')
...
g2 <- ggplot(aes(x= CreditScore, y = DebtToIncomeRatio, color = IncomeRange),
             data = subset(loan_verifiable_income, IncomeVerifiable == 'True' &
                             !is.na(CreditScore) &
                             !is.na(DebtToIncomeRatio) &
                             !is.na(IncomeRange))) +
  geom_jitter(alpha = 1/20) +
...
ggplot(aes(x= CreditScore, y = DebtToIncomeRatio, color = IncomeRange),
             data = subset(loan_2009, IncomeVerifiable == 'True' &
                             !is.na(CreditScore) &
                             !is.na(DebtToIncomeRatio) &
                             !is.na(IncomeRange))) +
  geom_jitter(alpha = 1/15, position = position_jitter()) +
  coord_cartesian(xlim = c(quantile(loan_2009$CreditScore,0.01,
                                    na.rm = T), 900),
                  ylim = c(0,quantile(loan_2009$DebtToIncomeRatio, 0.99,
                                      na.rm = T))) +
  scale_color_brewer(palette = "Spectral") +
...
ggplot(aes(x = CreditScore, y = BorrowerAPR, color = IncomeRange),
             data = subset(loan_verifiable_income, !is.na(CreditScore) &
                             ...
                             !is.na(IncomeRange))) +
  geom_jitter(alpha = 1/10) +
  coord_cartesian(xlim = c(quantile(loan_verifiable_income$CreditScore,0.01,
                                    na.rm = T), 900),
                  ylim = c(0,quantile(loan_verifiable_income$BorrowerAPR, 0.99,
                                      na.rm = T))) +
  scale_color_brewer(palette = "Spectral") +
...
#Facet by ProsperRiskRating
ggplot(aes(x = CreditScore, y = BorrowerAPR, color = IncomeRange),
             data = subset(loan_2009, !is.na(CreditScore) &
                             !is.na(BorrowerAPR) &
                             !is.na(IncomeRange))) +
  geom_jitter(alpha = 1/10) +
  coord_cartesian(xlim = c(quantile(loan_2009$CreditScore,0.01,
                                    na.rm = T), 900),
                  ylim = c(0,quantile(loan_2009$BorrowerAPR, 0.99,
                                      na.rm = T))) +
  scale_color_brewer(palette = "Spectral") +
  facet_wrap(~ProsperRiskRating) +
...
ggplot(aes(x=BorrowerState, y= LoanOriginalAmount, fill= IsBorrowerHomeowner),
  ...
  ggtitle('Median Loan Amount across State by IsBorrowerHomeowner facet LoanStanding') +
  facet_wrap(~LoanStanding) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, family = 'Times',
                                  face = 'bold.italic', size = 25),
        legend.position = c(1,1),
        legend.justification = c(1,1),
...
ggplot(aes(x=BorrowerState, y= LoanOriginalAmount, fill= IsBorrowerHomeowner),
  ...
  scale_fill_manual(values = c('firebrick2', 'springgreen3')) +
  ylab('Median Loan Original Amount') +
  ggtitle('Median Loan Amount across State by IsBorrowerHomeowner facet ProsperRiskRating') +
  facet_wrap(~ProsperRiskRating) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, family = 'Times',
                                  face = 'bold.italic', size = 25),
